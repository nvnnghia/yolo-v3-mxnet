...
from src.testdb import TestDB
from src.iterator import DetIter
...
class Detector(object):
    ...
    def show_result(self, img, dets, classes=[], thresh=0.2, fps=0):
        import random
	import cv2
        height = img.shape[0]
        width = img.shape[1]
	...
	font = cv2.FONT_HERSHEY_SIMPLEX
        for i in range(dets.shape[0]):
		    ...
		    cv2.putText(img,class_name,(xmin,ymin), font, 1,(0,255,0),2,cv2.LINE_AA)
	cv2.putText(img,"FPS= {0:.2f}".format(fps),(10,30), font, 1,(0,255,0),2,cv2.LINE_AA)
